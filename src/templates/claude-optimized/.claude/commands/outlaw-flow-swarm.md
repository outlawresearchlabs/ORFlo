...
npx outlaw-flow swarm "your complex task" --strategy <type> [options]

# Batch swarm operations
npx outlaw-flow swarm batch --tasks "frontend:dev,backend:dev,database:architect" --parallel
```

## Parallel Strategy Execution
```bash
# Execute multiple strategies concurrently
npx outlaw-flow swarm multi-strategy --config '{
...
npx outlaw-flow swarm batch-dev --services '{
...
npx outlaw-flow swarm batch-research --topics '{
...
npx outlaw-flow swarm batch-test --suites '{
...
npx outlaw-flow swarm pipeline --config '{
  "stages": [
...
npx outlaw-flow swarm analyze-pipeline --stages '{
...
npx outlaw-flow swarm deploy-multi --regions '{
...
npx outlaw-flow swarm federated-train --nodes 20 --parallel --aggregate-method "secure"
```
...
npx outlaw-flow swarm monitor-batch --metrics '{
...
npx outlaw-flow swarm auto-manage --rules '{
...
npx outlaw-flow swarm distribute --algorithm "weighted" --factors '{
  "agent-capacity": 0.4,
  "task-complexity": 0.3,
  "priority": 0.3
}'
...
npx outlaw-flow swarm batch-execute --resilient '{
...
npx outlaw-flow swarm batch --cache '{
...
npx outlaw-flow swarm optimize-network --compression "gzip" --batching 100 --keepalive
```
...
npx outlaw-flow swarm full-stack --project "e-commerce" --parallel-components '{
...
npx outlaw-flow swarm data-pipeline --config '{
...
npx outlaw-flow swarm security-audit --parallel-scans '{
