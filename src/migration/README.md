# Outlaw-Flow Migration System

A comprehensive migration system for existing outlaw-flow projects to adopt optimized prompts and configurations.
...
outlaw-flow migrate analyze --detailed --output analysis.json

# Check specific project
outlaw-flow migrate analyze /path/to/project
```
...
outlaw-flow migrate --dry-run --verbose

# Full migration
outlaw-flow migrate --strategy full
...
outlaw-flow migrate --strategy merge

# Force migration without prompts
outlaw-flow migrate --force

# Skip post-migration validation
outlaw-flow migrate --skip-validation
```
...
outlaw-flow migrate rollback --list

# Rollback to latest backup
outlaw-flow migrate rollback

# Rollback to specific backup
outlaw-flow migrate rollback --timestamp 2024-01-01T12:00:00

# Force rollback without confirmation
outlaw-flow migrate rollback --force
```
...
```bash
# Validate migration
outlaw-flow migrate validate

# Detailed validation report
outlaw-flow migrate validate --verbose

# Check project status
outlaw-flow migrate status
```
...
```bash
# Analyze first
outlaw-flow migrate analyze --detailed
...
outlaw-flow migrate --strategy merge --preserve-custom
...
outlaw-flow migrate --strategy selective
```
...
find . -name ".claude" -type d | while read dir; do
  ...
  outlaw-flow migrate "$project_path" --strategy selective --force
done
```
...
```typescript
import { MigrationRunner } from 'outlaw-flow/migration';
...
```typescript
import { MigrationValidator } from 'outlaw-flow/migration';
...
steps:
  - name: Analyze Migration
    run: outlaw-flow migrate analyze --output analysis.json
  
  - name: Run Migration
    run: outlaw-flow migrate --strategy selective --force
  
  - name: Validate Migration  
    run: outlaw-flow migrate validate
```
...
This migration system is part of outlaw-flow and follows the same license terms.
...
For more information, see the [main outlaw-flow documentation](../../README.md).