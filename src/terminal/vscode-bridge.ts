 ...
 * 
 * This file provides the bridge between Outlaw-Flow and VSCode extension API
 * for terminal management and output capture.
 * 
 * NOTE: This file is only used when Outlaw-Flow is packaged as a VS Code extension.
...
export function initializeTerminalBridge(context: vscode.ExtensionContext): void {
  // Inject VSCode API into global scope for Outlaw-Flow
  ...
  (vscode.window as any).createTerminal = function(options: vscode.TerminalOptions) {
    ...
    const match = options.name?.match(/Outlaw-Flow Terminal ([\w-]+)/);
    if (match) {
