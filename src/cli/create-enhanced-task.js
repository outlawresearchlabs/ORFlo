/**
 * Creates an enhanced task prompt with Outlaw-Flow guidance
...
1. **Memory Bank** (Always Available)
   ...
   - Export memory: \`npx outlaw-flow memory export <file>\` - Export memory to file
   - Import memory: \`npx outlaw-flow memory import <file>\` - Import memory from file
   - Memory stats: \`npx outlaw-flow memory stats\` - Show memory usage statistics

2. **System Management**
   - Check status: \`npx outlaw-flow status\` - View current system/task status
   - Monitor system: \`npx outlaw-flow monitor\` - Real-time system monitoring
   - List agents: \`npx outlaw-flow agent list\` - See active agents
   - List tasks: \`npx outlaw-flow task list\` - See active tasks
...
1. **Before Starting**:
   ...
   - Check memory stats: \`npx outlaw-flow memory stats\`
...
2. **During Execution**:
   ...
   ${flags.parallel ? '- Spawn agents: `npx outlaw-flow agent spawn researcher --name "research-agent"`' : ''}
   ${flags.parallel ? '- Create tasks: `npx outlaw-flow task create implementation "implement feature X"`' : ''}
   ${flags.parallel ? '- Assign tasks: `npx outlaw-flow task assign <task-id> <agent-id>`' : ''}
...
3. **Best Practices**:
   ...
   ${flags.research ? '- Store research findings: `npx outlaw-flow memory store research_findings "data"`' : ''}
...
Bash("npx outlaw-flow memory store task_analysis '{\\"status\\": \\"completed\\", \\"findings\\": [...]}'")
Bash("npx outlaw-flow memory stats")
Bash("npx outlaw-flow memory export backup.json")

# System Management
Bash("npx outlaw-flow status")
Bash("npx outlaw-flow monitor")  # Real-time monitoring
Bash("npx outlaw-flow agent list")
Bash("npx outlaw-flow task list --verbose")
${flags.parallel ? `
# Parallel Execution (enabled for this instance)
Bash("npx outlaw-flow agent spawn researcher --name research-bot")
Bash("npx outlaw-flow agent spawn coder --name code-bot")
Bash("npx outlaw-flow task create research 'Analyze best practices'")
Bash("npx outlaw-flow task create implementation 'Implement auth module'")
Bash("npx outlaw-flow task assign task-123 agent-456")` : ''}
${flags.research ? `
# Research Operations (research mode enabled)
# Use WebFetchTool for web research, then store findings
Bash("npx outlaw-flow memory store web_research_urls '[\\"url1\\", \\"url2\\"]'")
Bash("npx outlaw-flow memory store research_summary 'Key findings from research...'")` : ''}

# Configuration Management
Bash("npx outlaw-flow config show")
Bash("npx outlaw-flow config get orchestrator.maxConcurrentTasks")
Bash("npx outlaw-flow config set orchestrator.maxConcurrentTasks 20")

# Workflow Execution
Bash("npx outlaw-flow workflow examples/development-config.json")
Bash("npx outlaw-flow workflow examples/research-workflow.json --async")
\`\`\`
