...
- NEVER create files in node_modules/ or any outlaw-flow directories
...
\`\`\`bash
# Store your work
npx outlaw-flow memory store ${memoryNamespace}_<phase> "description of work completed"

# Query previous work
npx outlaw-flow memory query ${memoryNamespace}

# Examples for this task
npx outlaw-flow memory store ${memoryNamespace}_analysis "Analyzed ${taskDescription} - found X components needed"
npx outlaw-flow memory store ${memoryNamespace}_progress "Completed Y% of implementation"
npx outlaw-flow memory store ${memoryNamespace}_blockers "Issue with Z - need clarification"
\`\`\`
...
batchtool run --parallel \\
  "npx outlaw-flow sparc run architect 'design user authentication system' --non-interactive" \\
  "npx outlaw-flow sparc run security-review 'analyze authentication requirements' --non-interactive" \\
  "npx outlaw-flow sparc run spec-pseudocode 'create auth flow pseudocode' --non-interactive"

# Boomerang Pattern: Research → Design → Implement → Test → Refine
batchtool orchestrate --boomerang \\
  --phase1 "npx outlaw-flow sparc run ask 'research best auth practices' --non-interactive" \\
  --phase2 "npx outlaw-flow sparc run architect 'design based on research' --non-interactive" \\
  --phase3 "npx outlaw-flow sparc run code 'implement auth system' --non-interactive" \\
  --phase4 "npx outlaw-flow sparc run tdd 'test auth implementation' --non-interactive" \\
  --phase5 "npx outlaw-flow sparc run optimization 'refine auth performance' --non-interactive"

# Concurrent Feature Development
batchtool run --concurrent --max-parallel 3 \\
  "npx outlaw-flow sparc run code 'implement login feature' --non-interactive" \\
  "npx outlaw-flow sparc run code 'implement registration feature' --non-interactive" \\
  "npx outlaw-flow sparc run code 'implement password reset' --non-interactive" \\
  "npx outlaw-flow sparc run tdd 'create auth test suite' --non-interactive"
\`\`\`
...
1. **Focus on User's Project**
   - Build what they asked for, not improvements to outlaw-flow
...
Remember: You're building the user's project, using outlaw-flow only for memory and orchestration support.`;
}