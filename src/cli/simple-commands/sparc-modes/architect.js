...
1. **Requirements Analysis** (10 mins)
   ...
   - Query existing project context: \`npx outlaw-flow memory query ${memoryNamespace}\`
   ...
   - Store findings: \`npx outlaw-flow memory store ${memoryNamespace}_requirements "Core components: X, Y, Z. External deps: API-A, Service-B. Security: OAuth2, RLS policies needed."\`
...
2. **System Architecture Design** (20 mins)
   ...
   - Store architecture: \`npx outlaw-flow memory store ${memoryNamespace}_architecture "Microservices: auth-service, user-service, data-processor. APIs: REST for external, gRPC for internal. State: Event-sourced with CQRS."\`
...
3. **Technical Specifications** (15 mins)
   ...
   - Store specs: \`npx outlaw-flow memory store ${memoryNamespace}_tech_specs "Auth: JWT with refresh tokens. DB: PostgreSQL with read replicas. Cache: Redis. Config: Environment-based with secrets manager."\`
...
4. **Modular Implementation Plan** (10 mins)
   ...
   - Store plan: \`npx outlaw-flow memory store ${memoryNamespace}_implementation_plan "Phase 1: Core auth (tdd mode). Phase 2: User management (code mode). Phase 3: Integration (integration mode)."\`
...
- \`npx outlaw-flow sparc run spec-pseudocode "Create detailed pseudocode for ${taskDescription}" --non-interactive\`
- \`npx outlaw-flow sparc run security-review "Review architecture for vulnerabilities" --non-interactive\`

### Background Swarm Mode (for complex implementation):
- \`npx outlaw-flow swarm "Implement complete ${taskDescription} based on architecture" --strategy development --background --monitor\`
- \`npx outlaw-flow swarm "Create comprehensive test suite for ${taskDescription}" --strategy testing --background --parallel\`
- \`npx outlaw-flow swarm "Generate complete documentation for ${taskDescription}" --strategy research --background\`
...
npx outlaw-flow swarm "Research and analyze all architectural patterns for ${taskDescription}" \\
...
npx outlaw-flow swarm "Create architectural proof of concepts and prototypes for ${taskDescription}" \\
...
npx outlaw-flow swarm "Generate complete technical specifications and API documentation for ${taskDescription}" \\
...
npx outlaw-flow sparc run architect "Initial system design for ${taskDescription}" --non-interactive
npx outlaw-flow swarm "Refine architecture based on security and performance analysis" \\
...
npx outlaw-flow sparc run architect "Core system design" --non-interactive

# Phase 2: Parallel background research (Swarm)
npx outlaw-flow swarm "Deep dive research into scalability patterns" --strategy research --background &
npx outlaw-flow swarm "Security analysis and threat modeling" --strategy analysis --background &
npx outlaw-flow swarm "Performance benchmarking and optimization research" --strategy optimization --background &

# Phase 3: Implementation swarm (Background)
npx outlaw-flow swarm "Full implementation of ${taskDescription} architecture" \\
