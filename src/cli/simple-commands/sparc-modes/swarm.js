...
const SWARM_MODE = {
  ...
  // Prerequisites
  prerequisites: [
    "Outlaw-Flow initialized with --sparc flag",
...
## 🐝 SWARM CAPABILITIES
You have access to the advanced Outlaw-Flow swarm system with the following capabilities:
...
npx outlaw-flow swarm "Build a complete REST API with authentication" --strategy development --monitor

# Research swarm with distributed coordination
npx outlaw-flow swarm "Research and analyze blockchain technologies" --strategy research --distributed --ui

# Background optimization swarm (for long-running tasks)
npx outlaw-flow swarm "Optimize application performance across all modules" --strategy optimization --background --monitor
\`\`\`

### Advanced Swarm Configuration:
\`\`\`bash
# Full-featured swarm with all enterprise features
npx outlaw-flow swarm "Complex enterprise application development" \\
...
npx outlaw-flow swarm "Your complex task" --dry-run --strategy development
\`\`\`
...
npx outlaw-flow memory store "key" "value" --namespace swarm
npx outlaw-flow memory query "search-term" --namespace swarm
\`\`\`
...
- Monitor progress with \`npx outlaw-flow status\`
...
npx outlaw-flow swarm "Build microservices e-commerce platform" \\
...
npx outlaw-flow swarm "Comprehensive AI/ML market analysis" \\
  --strategy research --distributed --ui --verbose

# 2. Store findings for cross-agent access
npx outlaw-flow memory store "research-findings" "key insights" --namespace swarm
\`\`\`

### Long-Running Optimization:
\`\`\`bash
# 1. Background optimization swarm
npx outlaw-flow swarm "Optimize entire codebase performance" \\
...
### Common Issues:
1. **Swarm fails to start**: Check if Outlaw-Flow is initialized with \`npx outlaw-flow init --sparc\`
...
4. **Environment Preparation**
   \`\`\`bash
   # Ensure Outlaw-Flow is properly initialized
   npx outlaw-flow status
...
5. **Dry Run Validation**
   ...
   npx outlaw-flow swarm "${taskDescription}" --dry-run --strategy <selected-strategy>
   \`\`\`
...
6. **Launch Swarm**
   ...
   **For Standard Tasks (< 30 minutes):**
   \`\`\`bash
   npx outlaw-flow swarm "${taskDescription}" --strategy <strategy> --monitor --verbose
   \`\`\`

   **For Complex Tasks (30-60 minutes):**
   \`\`\`bash
   npx outlaw-flow swarm "${taskDescription}" --strategy <strategy> --parallel --monitor --review
   \`\`\`

   **For Long-Running Tasks (> 60 minutes):**
   \`\`\`bash
   npx outlaw-flow swarm "${taskDescription}" --strategy <strategy> --background --monitor --testing
   \`\`\`

   **For Enterprise Tasks (Multiple hours):**
   \`\`\`bash
   npx outlaw-flow swarm "${taskDescription}" \\
     --strategy <strategy> \\
     --mode distributed \\
     --max-agents 10 \\
     --background \\
...
8. **Cross-Agent Memory Sharing**
   ...
   npx outlaw-flow memory store "${memoryNamespace}_findings" "key insights and decisions"
   
   # Store progress updates
   npx outlaw-flow memory store "${memoryNamespace}_progress" "current completion status"
   
   # Store blockers or issues
   npx outlaw-flow memory store "${memoryNamespace}_blockers" "any impediments or questions"
   
   # Query shared knowledge
   npx outlaw-flow memory query "${memoryNamespace}" --limit 10
   \`\`\`
...
10. **Memory Documentation**
    \`\`\`bash
    # Store final results
    npx outlaw-flow memory store "${memoryNamespace}_final" "completed task summary and results"
    
    # Store lessons learned
    npx outlaw-flow memory store "${memoryNamespace}_lessons" "insights for future similar tasks"
    \`\`\`
