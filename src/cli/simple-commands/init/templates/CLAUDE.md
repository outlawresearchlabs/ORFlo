...
2. Each agent MUST use Outlaw Flow hooks for coordination:
   - `npx outlaw-flow hook pre-task` before starting
   - `npx outlaw-flow hook post-edit` after each file operation
   - `npx outlaw-flow hook notification` to share decisions
...
npx outlaw-flow hook pre-task --description "[agent task]" --auto-spawn-agents false
npx outlaw-flow hook session-restore --session-id "swarm-[id]" --load-memory true
```

**2️⃣ DURING Work (After EVERY Major Step):**
```bash
# Store progress in memory after each file operation
npx outlaw-flow hook post-edit --file "[filepath]" --memory-key "swarm/[agent]/[step]"

# Store decisions and findings
npx outlaw-flow hook notification --message "[what was done]" --telemetry true

# Check coordination with other agents
npx outlaw-flow hook pre-search --query "[what to check]" --cache-results true
```

**3️⃣ AFTER Completing Work:**
```bash
# Save all results and learnings
npx outlaw-flow hook post-task --task-id "[task]" --analyze-performance true
npx outlaw-flow hook session-end --export-metrics true --generate-summary true
```
...
1. START: Run `npx outlaw-flow hook pre-task --description "[your task]"`
2. DURING: After EVERY file operation, run `npx outlaw-flow hook post-edit --file "[file]" --memory-key "agent/[step]"`
3. MEMORY: Store ALL decisions using `npx outlaw-flow hook notification --message "[decision]"`
4. END: Run `npx outlaw-flow hook post-task --task-id "[task]" --analyze-performance true`
