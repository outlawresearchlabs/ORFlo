...
export class RecoveryManager {
  ...
  async recoverFromExecutableFailure(context) {
    ...
    try {
      ...
      if (createResult.success) {
        result.actions.push('Recreated outlaw-flow executable');
  ...
  async cleanupOldBackups() {
    ...
    try {
      const backupDir = `${this.workingDir}/.outlaw-flow-backups`;
...
`;
    ...
    try {
      await Deno.writeTextFile(`${this.workingDir}/outlaw-flow`, executableContent);
    } catch {
      result.success = false;
    }
