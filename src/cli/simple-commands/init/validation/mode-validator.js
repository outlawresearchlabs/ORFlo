...
export class ModeValidator {
  ...
  async checkSparcInitialization() {
    ...
    try {
      ...
      // Check for outlaw-flow executable
      try {
        const stat = await Deno.stat(`${this.workingDir}/outlaw-flow`);
        result.hasExecutable = stat.isFile;
      } catch {
        result.error = 'outlaw-flow executable not found';
      }
  ...
  async testModeAccess(modeName) {
    ...
    try {
      ...
      const command = new Deno.Command('./outlaw-flow', {
    ...
    try {
      ...
      if (success) {
        ...
        if (errorOutput.includes('dry-run') || errorOutput.includes('unknown flag')) {
          ...
          const testCommand = new Deno.Command('./outlaw-flow', {
            args: ['sparc', 'modes'],
            cwd: this.workingDir,
            stdout: 'piped',
            stderr: 'piped'
