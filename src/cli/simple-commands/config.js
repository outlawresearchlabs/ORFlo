...
async function initConfig(subArgs, flags) {
  ...
  const configFile = 'outlaw-flow.config.json';
  ...
  try {
    ...
    printSuccess('Initializing Outlaw-Flow configuration...');
    ...
    console.log('\nNext steps:');
    console.log('1. Review settings: outlaw-flow config show');
    console.log('2. Customize values: outlaw-flow config set <key> <value>');
    console.log('3. Validate config: outlaw-flow config validate');
...
async function showConfig(subArgs, flags) {
  ...
  try {
    ...
    if (format === 'json') {
      ...
      console.log(`   Path: ${config.memory?.path || './memory/outlaw-flow-data.json'}`);
      console.log('\n🤖 Agents:');
    ...
    console.log('Run "outlaw-flow config init" to create default configuration');
  }
}
...
async function getConfigValue(subArgs, flags) {
  ...
  if (!key) {
    ...
    console.log('  outlaw-flow config get terminal.poolSize');
    console.log('  outlaw-flow config get orchestrator.maxConcurrentTasks');
    return;
  }
  ...
  try {
    ...
    printError('Configuration file not found');
    console.log('Run "outlaw-flow config init" to create configuration');
  }
}
...
async function setConfigValue(subArgs, flags) {
  ...
  if (!key || value === undefined) {
    ...
    console.log('Examples:');
    console.log('  outlaw-flow config set terminal.poolSize 15');
    console.log('  outlaw-flow config set orchestrator.taskTimeout 600000');
    return;
  }
...
async function validateConfig(subArgs, flags) {
  ...
  try {
    ...
    console.log('Run "outlaw-flow config init" to create valid configuration');
  }
}
...
function showConfigHelp() {
  ...
  console.log();
  console.log('Examples:');
  console.log('  outlaw-flow config init');
  console.log('  outlaw-flow config set terminal.poolSize 15');
  console.log('  outlaw-flow config get orchestrator.maxConcurrentTasks');
  console.log('  outlaw-flow config validate');
}