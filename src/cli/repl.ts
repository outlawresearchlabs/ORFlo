...
/**
 * Enhanced Interactive REPL for Outlaw-Flow
 */
...
class CommandHistory {
  ...
  constructor(historyFile?: string) {
    this.historyFile = historyFile || '.outlaw-flow-history';
    this.loadHistory();
  }
...
async function connectToOrchestrator(context: REPLContext, target?: string): Promise<void> {
  ...
  if (success) {
    ...
    console.log(chalk.gray('Make sure Outlaw-Flow is running with: outlaw-flow start'));
  }
}
