...
export async function loadConfig(path = 'outlaw-flow.config.json') {
  const defaultConfig = {
    terminal: {
      poolSize: 10,
      recycleAfter: 20,
...
export async function saveConfig(config, path = 'outlaw-flow.config.json') {
...
// Outlaw Flow MCP integration helpers  
  ...
  try {
    ...
    const initMessage = {
      ...
      params: {
        ...
        clientInfo: { name: "outlaw-flow-cli", version: "2.0.0" }
      }
    };
