...
/**
 * Outlaw-Flow CLI - Core implementation using Node.js
 */
...
class CLI {
  ...
  private async loadConfig(configPath?: string): Promise<Record<string, unknown> | undefined> {
    const configFile = configPath || "outlaw-flow.config.json";
    try {
...
async function main() {
  if (process.argv[1] && (process.argv[1].endsWith('cli-core.js') || process.argv[1].endsWith('cli-core.ts'))) {
    const cli = new CLI("outlaw-flow", "Advanced AI Agent Orchestration System");
