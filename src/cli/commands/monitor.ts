...
/**
 * Monitor command for Outlaw-Flow - Live dashboard mode
 */
...
class Dashboard {
  ...
  private renderHeader(data: MonitorData): void {
    ...
    console.log(chalk.cyan.bold('Outlaw-Flow Live Monitor') + chalk.gray(` - ${time}`));
  ...
  private renderError(error: any): void {
    ...
    if ((error as Error).message.includes('ECONNREFUSED')) {
      console.log(chalk.red('✗ Cannot connect to Outlaw-Flow'));
  ...
  private async checkSystemRunning(): Promise<boolean> {
    try {
      return await existsSync('.outlaw-flow.pid');
    } catch {
      return false;
    }
  }
