...
/**
 * Status command for Outlaw-Flow
 */
...
export const statusCommand = new Command()
  .name('status')
  .description('Show Outlaw-Flow system status')
...
async function showStatus(options: any): Promise<void> {
  try {
    ...
    if ((error as Error).message.includes('ECONNREFUSED') || (error as Error).message.includes('connection refused')) {
      console.error(chalk.red('✗ Outlaw-Flow is not running'));
      console.log(chalk.gray('Start it with: outlaw-flow start'));
    } else {
...
async function watchStatus(options: any): Promise<void> {
  ...
  console.log(chalk.cyan('Watching Outlaw-Flow status...'));
  ...
  while (true) {
    ...
    console.clear();
    console.log(chalk.cyan.bold('Outlaw-Flow Status Monitor'));
