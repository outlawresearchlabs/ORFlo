...
Comprehensive task management with orchestration features for Outlaw-Flow. Integrates seamlessly with TodoWrite/TodoRead for coordination and Memory for state persistence.
...
outlaw-flow task create development "Implement authentication" \
  --priority 80 \
...
outlaw-flow task status task-789 \
  --show-logs \
  --show-checkpoints \
  --show-metrics \
  --show-dependencies \
...
outlaw-flow task cancel task-789 \
...
```bash
# Create workflow
outlaw-flow task workflow create "E-commerce Platform" \
...
outlaw-flow task workflow execute workflow-123 \
  --variables '{"environment":"staging"}' \
  --monitor

# Visualize dependency graph
outlaw-flow task workflow visualize workflow-123 \
  --format dot \
