...
```bash
npx --yes outlaw-flow@2.0.0 hive-mind
```

4. **Global installation**:
```bash
npm install -g outlaw-flow@2.0.0
outlaw-flow hive-mind
```
...
```bash
npx outlaw-flow@2.0.0 hive-mind --non-interactive \
  --task "Your task here" \
  --complexity medium
```
...
3. **Try web UI**:
```bash
npx outlaw-flow@2.0.0 start --ui
...
winpty npx outlaw-flow@2.0.0 hive-mind
```
...
npx outlaw-flow@2.0.0 hive-mind init --topology mesh
```
...
```bash
npx outlaw-flow@2.0.0 hive-mind config get max-agents
# Increase if needed
...
2. **Verify resources**:
```bash
npx outlaw-flow@2.0.0 hive-mind monitor --metrics "resources"
```

3. **Manual spawn**:
```bash
npx outlaw-flow@2.0.0 hive-mind agents spawn \
  --type coordinator \
  --name "Manual-Coord"
```
...
npx outlaw-flow@2.0.0 hive-mind swarm scale --target 4
...
4. **Enable caching**:
```bash
npx outlaw-flow@2.0.0 hive-mind config set cache.enabled true
npx outlaw-flow@2.0.0 hive-mind config set cache.ttl 3600
```
...
```bash
npx outlaw-flow@2.0.0 hive-mind report performance \
  --metrics "token-usage" \
  --timeframe 24h
```
...
2. **Set token limits**:
```bash
npx outlaw-flow@2.0.0 hive-mind config set \
...
1. **Clear old entries**:
```bash
npx outlaw-flow@2.0.0 hive-mind memory clear \
  --older-than 7d
```
...
4. **Backup and reset**:
```bash
npx outlaw-flow@2.0.0 hive-mind memory backup \
  --path ./backup.json
npx outlaw-flow@2.0.0 hive-mind memory clear --all
```
...
```bash
npx outlaw-flow@2.0.0 hive-mind memory verify
```

2. **Restore from backup**:
```bash
npx outlaw-flow@2.0.0 hive-mind memory restore \
  --path ./last-good-backup.json
```

3. **Rebuild memory index**:
```bash
npx outlaw-flow@2.0.0 hive-mind memory rebuild-index
```
...
```bash
npx outlaw-flow@2.0.0 hive-mind --task \
...
2. **Use templates**:
```bash
npx outlaw-flow@2.0.0 hive-mind template use rest-api
```
...
1. **Check task status**:
```bash
npx outlaw-flow@2.0.0 hive-mind task status
```

2. **Review agent logs**:
```bash
npx outlaw-flow@2.0.0 hive-mind agents list --with-logs
```

3. **Re-run failed tasks**:
```bash
npx outlaw-flow@2.0.0 hive-mind task retry --failed-only
```

4. **Generate report**:
```bash
npx outlaw-flow@2.0.0 hive-mind report tasks \
  --format detailed \
  --include-errors
```
...
```bash
claude mcp list
# Should show outlaw-flow with 87 tools
```

2. **Re-add MCP server**:
```bash
claude mcp remove outlaw-flow
claude mcp add outlaw-flow npx outlaw-flow@2.0.0 mcp start --stdio
```
...
```bash
npx outlaw-flow@2.0.0 hive-mind integrate github \
  --test-connection
```
...
1. **Run with TTY**:
```bash
docker run -it outlaw-flow:2.0.0 hive-mind
```

2. **Without TTY**:
```bash
docker run outlaw-flow:2.0.0 hive-mind \
  --non-interactive \
  --no-color \
  --task "Your task"
```
...
# Check debug logs
ls ~/.outlaw-flow/logs/
cat ~/.outlaw-flow/logs/hive-mind-debug.log

# Agent specific logs
cat ~/.outlaw-flow/logs/agents/*.log
```
...
npx outlaw-flow@2.0.0 hive-mind report profile \
  --last-run
```
...
```bash
npx outlaw-flow@2.0.0 hive-mind --help
npx outlaw-flow@2.0.0 hive-mind <command> --help
```

### 2. Diagnostics

```bash
npx outlaw-flow@2.0.0 hive-mind diagnostic run
```
...
- Discord: https://discord.gg/outlaw-flow
