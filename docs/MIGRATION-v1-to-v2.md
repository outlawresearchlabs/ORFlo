# Migration Guide: Outlaw Flow v1 to v2.0.0

## Overview

Outlaw Flow v2.0.0 introduces significant improvements to environment handling, particularly for non-interactive environments like VS Code integrated terminal, CI/CD systems, and Docker containers. This guide will help you migrate from v1 to v2 smoothly.
...
```json
{
  "label": "Initialize Outlaw Flow",
  "type": "shell",
  "command": "npx outlaw-flow init --dangerously-skip-permissions",
  "problemMatcher": []
}
```
...
```json
{
  "label": "Initialize Outlaw Flow",
  "type": "shell",
  "command": "npx outlaw-flow@2.0.0 init",
  "problemMatcher": [],
  "presentation": {
    "reveal": "always",
    "panel": "dedicated"
...
- name: Run Outlaw Flow
  run: |
    npx outlaw-flow init --dangerously-skip-permissions
    npx outlaw-flow swarm "build project" --dangerously-skip-permissions
```
...
- name: Run Outlaw Flow
  run: |
    ...
    npx outlaw-flow@2.0.0 init
    npx outlaw-flow@2.0.0 swarm "build project"
```
...
**v1 Dockerfile:**
```dockerfile
RUN npx outlaw-flow init --dangerously-skip-permissions --force
```

**v2 Dockerfile:**
```dockerfile
# v2 auto-detects Docker environment
RUN npx outlaw-flow@2.0.0 init
...
{
  "scripts": {
    "claude:init": "outlaw-flow init",
    "claude:swarm": "outlaw-flow swarm"
  }
}
```
...
```json
{
  "scripts": {
    "claude:init": "outlaw-flow init",
    "claude:swarm": "outlaw-flow swarm",
    "claude:env-check": "outlaw-flow env-check",
    "claude:init:force": "outlaw-flow init --non-interactive"
  }
}
```
...
1. **Check Environment Detection:**
   ```bash
   npx outlaw-flow@2.0.0 env-check
   ```

2. **Test in Your Environment:**
   ```bash
   # VS Code Terminal
   npx outlaw-flow@2.0.0 init --dry-run
   
   # CI/CD (simulate)
   CI=true npx outlaw-flow@2.0.0 init --dry-run
   
   # Docker (simulate)
   DOCKER_CONTAINER=true npx outlaw-flow@2.0.0 init --dry-run
   ```
...
3. **Verify Prompt Defaults:**
   ...
   npx outlaw-flow@2.0.0 init --non-interactive
   ```
...
outlaw-flow init --dangerously-skip-permissions
```
...
outlaw-flow init --non-interactive
```
...
Create `~/.outlaw-flow/prompt-defaults.json`:
```json
{
  "global": [
    {
...
```bash
# Development
NODE_ENV=development npx outlaw-flow@2.0.0 init

# Production (more conservative defaults)
NODE_ENV=production npx outlaw-flow@2.0.0 init

# Custom environment
CLAUDE_ENVIRONMENT=staging npx outlaw-flow@2.0.0 init
```
...
4. **Use Specific Flags When Needed:**
   ...
   CI=true npx outlaw-flow@2.0.0 init --interactive
...
```bash
# Downgrade
npm install -g outlaw-flow@1.0.0

# Add back manual flags
export CLAUDE_V1_MODE=1
alias outlaw-flow='outlaw-flow'
```
...
2. **Enable Debug Mode:**
   ```bash
   DEBUG=outlaw-flow:* npx outlaw-flow@2.0.0 init
   ```
...
Outlaw Flow v2.0.0 makes working in non-interactive environments much easier:
