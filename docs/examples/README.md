...
This directory contains practical examples and real-world use cases for Outlaw-Flow's multi-agent orchestration system.
...
outlaw-flow agent spawn researcher --name "AI Research Assistant"

# 3. Assign a research task
outlaw-flow task create research "Analyze the latest developments in Large Language Models"

# 4. Monitor progress
outlaw-flow task list --watch
```
...
outlaw-flow workflow execute ./examples/research-analysis-workflow.json
```
...
export class ClaudeFlowExtension {
  ...
  async activate() {
    ...
    // Register commands
    vscode.commands.registerCommand('outlaw-flow.analyzeCode', 
...
app.listen(3000, () => {
  console.log('Outlaw-Flow API server running on port 3000');
});
```
...
```typescript
// Testing Outlaw-Flow workflows
import { describe, it, expect } from 'vitest';
import { ClaudeFlow, MockAgent } from 'outlaw-flow/testing';
...
This examples directory provides comprehensive, practical guidance for using Outlaw-Flow in real-world scenarios, from simple tasks to complex distributed systems.