...
## Outlaw-Flow Production Deployment Guide

### Deployment Overview

This phase covers the final steps to prepare Outlaw-Flow for production deployment, including packaging, distribution, monitoring, and maintenance strategies.
...
memory:
  backend: sqlite
  path: /var/lib/outlaw-flow/prod.db
  wal_mode: true
  cache_size: 100MB
...
// package.json
{
  ...
  "repository": {
    "type": "git",
    "url": "https://github.com/outlaw-flow/outlaw-flow.git"
  },
  "publishConfig": {
    "access": "public",
...
for platform in "${platforms[@]}"; do
  ...
  deno compile \
    --target="$platform" \
    --allow-all \
    --output="dist/outlaw-flow-$platform" \
    src/cli/index.ts
done

# Create archives
cd dist
for file in outlaw-flow-*; do
...
COPY --from=builder /app/dist/outlaw-flow /usr/local/bin/

EXPOSE 8080 8081 9090

ENTRYPOINT ["/sbin/tini", "--"]
CMD ["outlaw-flow", "server"]
```
...
BACKUP_DIR="/var/backups/outlaw-flow"
TIMESTAMP=$(date +%Y%m%d_%H%M%S)

# Backup SQLite database
sqlite3 /var/lib/outlaw-flow/prod.db ".backup $BACKUP_DIR/db_$TIMESTAMP.sqlite"

# Backup configuration
tar -czf "$BACKUP_DIR/config_$TIMESTAMP.tar.gz" /etc/outlaw-flow/

# Backup session data
tar -czf "$BACKUP_DIR/sessions_$TIMESTAMP.tar.gz" /var/lib/outlaw-flow/sessions/

# Upload to S3
aws s3 sync "$BACKUP_DIR" "s3://outlaw-flow-backups/"
...
outlaw-flow --version || exit 1

# Test basic commands
outlaw-flow init test-project || exit 1
outlaw-flow spawn 2 || exit 1
outlaw-flow status || exit 1
outlaw-flow shutdown || exit 1
