...
# Using outlaw-flow wrapper
./bin/outlaw-flow swarm <objective> [options]
...
```bash
# Basic swarm
./bin/outlaw-flow swarm "Build a REST API"

# Complex migration with coordinator
./bin/outlaw-flow swarm "Migrate monolithic app to microservices" \
  --coordinator --max-agents 10 --review

# Research swarm
./bin/outlaw-flow swarm "Research best practices for cloud architecture" \
  --strategy research --research --max-agents 8

# Development swarm with parallel execution
./bin/outlaw-flow swarm "Implement user authentication system" \
  --strategy development --parallel --review

# Preview configuration
./bin/outlaw-flow swarm "Create mobile app" --dry-run --verbose
```
