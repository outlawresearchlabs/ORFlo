...
This document describes the complete MCP implementation for Outlaw-Flow, providing a production-ready interface for AI tool integration.
...
- **Comprehensive Tools**: Full Outlaw-Flow functionality exposure
...
### 7. Outlaw-Flow Tools (`src/mcp/outlaw-flow-tools.ts`)

Complete set of tools exposing Outlaw-Flow functionality.
...
```bash
# Via CLI
outlaw-flow mcp-call agents/spawn '{"type": "researcher", "name": "Research Assistant"}'
...
```bash
# Start MCP server
outlaw-flow start --mcp-transport stdio

# Client connection
echo '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{...}}' | outlaw-flow mcp
```
...
```bash
# Start HTTP server
outlaw-flow start --mcp-transport http --port 3000
...
outlaw-flow start --log-level debug --mcp-transport http
```
...
1. **Add new tools** in `src/mcp/outlaw-flow-tools.ts`
...
This MCP implementation is part of Outlaw-Flow and follows the same MIT license.