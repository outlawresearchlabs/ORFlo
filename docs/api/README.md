...
Comprehensive programmatic API documentation for Outlaw-Flow's multi-agent orchestration system.

## Overview

Outlaw-Flow provides a rich TypeScript/JavaScript API for programmatic control of the multi-agent system. This API enables deep integration with existing applications and custom workflow automation.
...
```bash
# Install Outlaw-Flow as a dependency
npm install outlaw-flow
...
```typescript
import { ClaudeFlow, ClaudeFlowConfig } from 'outlaw-flow';
...
// Create and start Outlaw-Flow instance
...
claudeFlow.once('system.started', () => {
  console.log('Outlaw-Flow system is ready');
});
```
...
import { ClaudeFlow } from 'outlaw-flow';

const app = express();
const claudeFlow = new ClaudeFlow();

// Middleware to inject Outlaw-Flow instance
...
export const ClaudeFlowProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  if (!isReady) {
    return <div>Initializing Outlaw-Flow...</div>;
  }
...
This comprehensive API reference provides complete documentation for programmatic integration with Outlaw-Flow, enabling developers to build sophisticated multi-agent applications and custom workflows.