...
/**
 * Remote NPX Execution Tests for Outlaw Flow Migration
 * Tests that 'npx outlaw-flow@latest' works correctly
 */
...
// Test configuration
const TEST_DIR = path.join(os.tmpdir(), 'outlaw-flow-npx-test-' + Date.now());
...
async function checkNpmPackage() {
  try {
    const output = exec(`npm view outlaw-flow version`, { cwd: process.cwd() });
    return output.trim();
  } catch (error) {
    return null;
  }
...
if (!npmVersion) {
  console.log(`${colors.yellow}Warning: outlaw-flow not found on npm registry${colors.reset}`);
...
runTest('NPX executes without install', () => {
  ...
  const output = exec('npx outlaw-flow --version', { cwd: process.cwd() });
...
runTest('NPX help command works', () => {
  const output = exec('npx outlaw-flow --help', { cwd: process.cwd() });
...
runTest('NPX init creates files', () => {
  exec('npx outlaw-flow init -y');
  
  // Check files were created
  if (!fs.existsSync(path.join(TEST_DIR, 'CLAUDE.md'))) {
    throw new Error('CLAUDE.md not created');
  }
  if (!fs.existsSync(path.join(TEST_DIR, 'outlaw-flow'))) {
    throw new Error('outlaw-flow script not created');
  }
});
...
runTest('Cross-platform NPX execution', () => {
  ...
  const output = exec(`${command} outlaw-flow --version`, { cwd: process.cwd() });
  if (!output.includes('outlaw-flow')) {
...
runTest('NPX with specific version (if published)', () => {
  ...
  const output = exec(`npx outlaw-flow@${npmVersion} --version`);
...
runTest('NPX in subdirectory', () => {
  ...
  const output = exec('npx outlaw-flow --help', { cwd: subdir });
...
runTest('NPX with environment variables', () => {
  const output = exec('npx outlaw-flow config get github.token', {
...
runTest('NPX cache behavior', () => {
  ...
  exec('npx outlaw-flow --version', { cwd: process.cwd() });
...
runTest('NPX with npm scripts', () => {
  const packageJson = {
    ...
    version: '1.0.0',
    scripts: {
      'test-claude': 'npx outlaw-flow --version'
    }
  };
...
runTest('Global vs local precedence', () => {
  // Create a local package.json with outlaw-flow
  const packageJson = {
    name: 'local-test',
    version: '1.0.0',
    devDependencies: {
      'outlaw-flow': 'file:' + path.resolve(__dirname, '../../..')
    }
  };
  ...
  const output = exec('npx outlaw-flow --version');
  if (!output.includes('outlaw-flow')) {
