# Outlaw Flow Migration Test Suite

Comprehensive test suite for validating the Outlaw Flow migration from Deno to pure Node.js/npm.

## Test Categories

### 1. Local Execution Tests (`local/`)
Tests that Outlaw Flow works correctly with pure Node.js without any Deno dependencies.
...
Validates that `npx outlaw-flow@latest` works correctly.
...
- Push to main or outlaw-flow-v2.0.0 branches
- Pull requests
...
```bash
docker build -t outlaw-flow:latest -f Docker/Dockerfile .
```
...
### Testing in Docker
```bash
docker run --rm outlaw-flow:latest --help
docker run --rm outlaw-flow:latest init -y
```
