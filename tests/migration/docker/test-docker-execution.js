...
/**
 * Docker Execution Tests for Outlaw Flow Migration
...
// Test configuration
const DOCKER_IMAGE = 'outlaw-flow:test';
...
runTest('Docker Compose stack starts', () => {
  try {
    ...
    exec(`docker-compose -f ${COMPOSE_FILE} up -d outlaw-flow mcp-server`, {
    ...
    if (!psOutput.includes('outlaw-flow') || !psOutput.includes('mcp-server')) {
...
runTest('Container networking works', () => {
  try {
    ...
    exec('docker network create test-outlaw-flow-net');
    
    // Run two containers on the same network
    exec(`docker run -d --name test-server --network test-outlaw-flow-net ${DOCKER_IMAGE} start --mode server`);
    execSync('sleep 2');
    
    const output = exec(
      `docker run --rm --network test-outlaw-flow-net ${DOCKER_IMAGE} config set mcp.server http://test-server:3000`
    );
    ...
    try {
      ...
      exec('docker network rm test-outlaw-flow-net');
    } catch (e) {
      // Ignore cleanup errors
    }
  }
