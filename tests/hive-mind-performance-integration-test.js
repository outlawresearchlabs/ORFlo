...
class HiveMindPerformanceIntegrationTest {
  ...
  async testSwarmCreationMetrics() {
    ...
    try {
      ...
      const swarmInitResult = await this.executeCommand('mcp__outlaw-flow__swarm_init', {
        topology: 'mesh',
        maxAgents: 8,
        strategy: 'auto'
      });
      ...
      if (swarmInitResult.success) {
        ...
        const swarmStatus = await this.executeCommand('mcp__outlaw-flow__swarm_status', {
  ...
  async testAgentSpawningPerformance() {
    ...
    for (const agentType of agentTypes) {
      ...
      try {
        const result = await this.executeCommand('mcp__outlaw-flow__agent_spawn', {
          type: agentType,
          swarmId,
  ...
  async testTaskOrchestrationMetrics() {
    ...
    for (let i = 0; i < tasks.length; i++) {
      ...
      try {
        const result = await this.executeCommand('mcp__outlaw-flow__task_orchestrate', {
          task: tasks[i],
  ...
  async testRealSwarmPerformance() {
    ...
    for (const phase of scenario.phases) {
      ...
      for (const agentType of phase.agents) {
        try {
          const agent = await this.executeCommand('mcp__outlaw-flow__agent_spawn', {
            type: agentType,
      ...
      for (let i = 0; i < phase.tasks; i++) {
        ...
        try {
          const task = await this.executeCommand('mcp__outlaw-flow__task_orchestrate', {
  ...
  async testMemoryUsageTracking() {
    ...
    for (const operation of operations) {
      try {
        const result = await this.executeCommand('mcp__outlaw-flow__memory_usage', {
          action: 'store',
  ...
  async testPerformanceReports() {
    ...
    for (const format of reportTypes) {
      ...
      try {
        const report = await this.executeCommand('mcp__outlaw-flow__performance_report', {
          format,
          timeframe: '24h'
        });
  ...
  async executeCommand(tool, params) {
    ...
    switch (tool) {
      case 'mcp__outlaw-flow__swarm_init':
        return {
          success: true,
      ...
      case 'mcp__outlaw-flow__swarm_status':
        return {
          success: true,
      ...
      case 'mcp__outlaw-flow__agent_spawn':
        return {
          success: true,
      ...
      case 'mcp__outlaw-flow__task_orchestrate':
        return {
      ...
      case 'mcp__outlaw-flow__memory_usage':
        return {
          success: true,
      ...
      case 'mcp__outlaw-flow__performance_report':
        return {
          success: true,
  ...
  async cleanup() {
    ...
    for (const swarmId of this.swarmIds) {
      try {
        await this.executeCommand('mcp__outlaw-flow__swarm_destroy', { swarmId });
