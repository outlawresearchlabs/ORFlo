...
Deno.test('Batch Initialization - Minimal', async () => {
  ...
  for (const project of projects) {
    ...
    await Deno.stat(`${project}/memory/outlaw-flow-data.json`);
  }
...
Deno.test('Multi-Environment Initialization', async () => {
  ...
  const devData = JSON.parse(await Deno.readTextFile('multi-env-app-dev/memory/outlaw-flow-data.json'));
  const stagingData = JSON.parse(await Deno.readTextFile('multi-env-app-staging/memory/outlaw-flow-data.json'));
