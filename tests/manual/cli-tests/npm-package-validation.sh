...
test_package_structure() {
    ...
    run_test "Entry point exists" "[ -f cli.js ] || [ -f bin/outlaw-flow ]"
...
test_local_installation() {
    ...
    run_test "Install outlaw-flow locally" "npm install /workspaces/ruv-FANN/outlaw-flow/outlaw-flow --no-save"
    run_test "Binary is symlinked" "[ -L node_modules/.bin/outlaw-flow ] || [ -f node_modules/.bin/outlaw-flow ]"
    run_test "Can execute via npx" "timeout 10 npx outlaw-flow --version | grep -E '[0-9]+\.[0-9]+\.[0-9]+'"
    run_test "Can execute via direct path" "timeout 10 ./node_modules/.bin/outlaw-flow --version | grep -E '[0-9]+\.[0-9]+\.[0-9]+'"
...
test_global_installation() {
    ...
    run_test "Global install simulation" "npm install /workspaces/ruv-FANN/outlaw-flow/outlaw-flow -g --prefix $NPM_CONFIG_PREFIX"
    run_test "Global binary exists" "[ -f $NPM_CONFIG_PREFIX/bin/outlaw-flow ]"
    run_test "Global binary is executable" "[ -x $NPM_CONFIG_PREFIX/bin/outlaw-flow ]"
    
    # Test execution (add to PATH temporarily)
    export PATH="$NPM_CONFIG_PREFIX/bin:$PATH"
    run_test "Can execute global binary" "timeout 10 outlaw-flow --version | grep -E '[0-9]+\.[0-9]+\.[0-9]+'"
