...
# Automated CI/CD Test Runner for outlaw-flow
...
run_core_tests() {
    ...
    # Test 8: Binary Files
    if execute_test_with_retry "Binary Files" "[ -f bin/outlaw-flow ] && [ -x bin/outlaw-flow ]"; then
        ((tests_passed++))
...
<testsuites name="outlaw-flow-cli-tests" tests="$total_tests" failures="$tests_failed" time="$duration">
...
- **Binary:** $(node -pe 'JSON.parse(require("fs").readFileSync("package.json")).bin["outlaw-flow"]' 2>/dev/null)
