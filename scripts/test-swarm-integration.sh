#!/bin/bash
# Integration test for Outlaw-Flow Swarm Mode

echo "Outlaw-Flow Swarm Mode Integration Test"
echo "======================================="
echo
...
if [ -f "./bin/outlaw-flow" ]; then
    echo "✅ outlaw-flow binary found"
else
    echo "❌ outlaw-flow binary not found"
    exit 1
fi

if [ -f "./bin/outlaw-flow-swarm" ]; then
    echo "✅ outlaw-flow-swarm wrapper found"
else
    echo "❌ outlaw-flow-swarm wrapper not found"
    exit 1
fi

# Test 2: Help command
echo -e "\nTest 2: Testing help command..."
if ./bin/outlaw-flow help swarm | grep -q "Claude Swarm Mode"; then
...
if ./bin/outlaw-flow --help | grep -q "swarm"; then
...
if ./bin/outlaw-flow-swarm "Test objective" --dry-run | grep -q "DRY RUN"; then
...
OUTPUT=$(./bin/outlaw-flow-swarm "Complex test" --strategy research --max-agents 10 --coordinator --review --parallel --dry-run)
...
echo "  1. ./bin/outlaw-flow swarm \"Your objective\" [options]"
echo "  2. ./bin/outlaw-flow-swarm \"Your objective\" [options]"
echo "  3. npx outlaw-flow swarm \"Your objective\" [options]"
echo
echo "For more info: ./bin/outlaw-flow help swarm"